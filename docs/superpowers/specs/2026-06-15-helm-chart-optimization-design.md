...
> Note: a separate, narrower correctness pass shipped in parallel as
> `2026-06-15-helm-chart-defect-fix-design.md` (PR #4). This optimization
> supersedes/extends it: the checksum-based rollout replaces the timestamp churn,
> and PR #4's opt-in `deployTimestamp.enabled` gate is preserved as a complement.
...
- **Replace timestamp churn**: remove the `now`-based annotation from
  ...
  annotations are always emitted, merged with any user podAnnotations. The
  opt-in `deployTimestamp.enabled` gate (from PR #4, default `false`) is retained
  for operators who want force-rollout-on-every-upgrade.
...
- Extend `.github/workflows-helm-chart/ci.yml`: run `helm lint` + `helm template`
  ...
  just staging, via a shared `scripts/verify-render.sh`. Assertions on render:
...
- `readOnlyRootFilesystem: true` is the one behavioral risk: a model server that
  ...
  common case; operators with other writable paths add `volumeMounts`. Surfaced in
  `NOTES.txt` at install time.

## Verification

- `helm lint Helm-Chart/mychart -f <each values file>` passes.
- `helm template` renders cleanly for base, staging, production, canary.
- `scripts/verify-render.sh` assertions pass against the rendered output.
- Spot-check: rendering twice with identical inputs produces identical output
  (proves the timestamp churn is gone; `deployTimestamp` is off by default).
